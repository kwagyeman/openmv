    ...
    for (y = 0; y < cy && bContinue; y++) //, jd.y += mcuCY)
    {
        for (x = 0; x < cx && bContinue && iErr == 0; x++)
        {
            ...
            else
            {
                ...
                {
                    case 0x00: // grayscale (drawn to a RGB565 surface)
...
void imlib_jpeg_decompress_image_to_binary(image_t *dst, image_t *src)
{
    ...
    memset(dst->data, 0, image_size(dst)/8); // fill it with with 0's so we only need to write "set" bits
    if (rc) { // success
...
//            memset(dst->data, 0, dst->w * dst->h); // decode error, fill it with with 0's
            return;
        }
...
//            printf("Decode failure!\n");
            memset(dst->data, 0, image_size(dst) * 2); // decode error, fill with 0's
            return;
        }
    } else  { // failed to parse the header
//        printf("Error opening the JPEG compressed data\n");
        return;
    }
#if (TIME_JPEG==1)
    printf("time: %u ms\n", mp_hal_ticks_ms() - start);

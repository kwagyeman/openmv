...
class rtsp_server:
    def __init__(self, network_if, port=554):  # private
        ...
        self._sock = None  # control socket (also carries interleaved RTP)
        self._writer = None
        self._sending = False
    ...
    async def _send_all(self, data):  # private
        # All writes (control and interleaved RTP) are serialized by the
        # __sending flag, so a control response can never split an RTP
        # packet. On MicroPython the raw socket is driven directly (no
        # stream-buffer allocations); CPython forbids that, so host runs
        # go through the StreamWriter.
        while self._sending:
            await _sleep_ms(1)
        self._sending = True
        try:
            ...
            sock = self._sock
            if sock is None:  # CPython host path
                w.write(bytes(data))
                await w.drain()
                return
            off = 0
    ...
    async def _handle_client(self, reader, writer):  # private
        if self._writer is not None:
            writer.close()
            return
        self._sock = getattr(writer, "s", None)  # MicroPython raw socket
        self._writer = writer

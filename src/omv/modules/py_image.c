...
static mp_obj_t py_image_midpoint(uint n_args, const mp_obj_t *pos_args, mp_map_t *kw_args) {
    ...
    if (bias < 0.0f || bias > 1.0f) {
        mp_raise_msg(&mp_type_ValueError, MP_ERROR_TEXT("0 <= bias <= 1"));
    }
    ...
    imlib_midpoint_filter(image, ksize, bias, args[ARG_threshold].u_bool,

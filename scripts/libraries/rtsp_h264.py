...
class rtsp_server(rtsp.rtsp_server):
    ...
    async def _send_rtp_l16(self, pcm, timestamp):  # private
        # L16 is big-endian; the mic produces little-endian, so swap into
        # the packet buffer.
        pkt = self._pkt
        n = len(pcm)
        i = 0
        while i < n:
            ...
            self._rtp_header(0x61, self._audio_sequence_number,  # PT 97
                             timestamp + (i >> 1) // self._audio_channels, 1)
    ...
    def _make_audio_loop(self, audio_callback,
        ...
        self._audio_rate = audio_rate if audio_callback else 0
    ...
    async def stream(self, image_callback, quality=90, audio_callback=None,
                     audio_rate=16000, audio_channels=1):  # public
        """Stream MJPEG video (and optional L16 PCM audio) forever."""
        self._h264 = False
        await super().stream(image_callback, quality,
                             _audio_loop=self._make_audio_loop(
                                 audio_callback, audio_rate, audio_channels))

    async def stream_h264(self, encoder, image_callback, audio_callback=None,
                          audio_rate=16000, audio_channels=1):  # public
        """Stream H.264 from a codec.H264Encoder (and optional L16 PCM
        audio) forever. The encoder's SPS/PPS go out in the SDP and are
        repeated in-band at every keyframe for late joiners."""
        self._h264 = True
